<template>
  <v-row align="center" justify="center">
    ...
    <v-col cols="10" sm="5" v-for="goal in goals" :key="goal.id">
      <v-card v-if="goal.inProgress" class="mx-auto">
        ...
        <v-card-actions>
          ...
          <v-btn @click="clickDelete(goal.id)">삭제</v-btn>
        </v-card-actions>
      </v-card>
    </v-col>
    ...
    <v-col cols="10">
      <v-dialog v-model="dialog" max-width="290">
        <v-card>
          ...
          <v-card-actions>
            <v-spacer> </v-spacer>
            <v-btn color="green darken-1" text @click="acceptDelete(clickedId)">
              삭제
            </v-btn>
